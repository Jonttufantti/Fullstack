    ...
    if (confirm(message)) {
      personsService
        ...
        .then(() => {
          ...
          setNotification({ text: `The person '${name}' was succesfully removed from server`, type: `success` })
        })
        .catch(error => {
          alert()
          setPersons(persons.filter(p => p.id !== id))
          setNotification({ text: `The person '${name}' was already removed from server`, type: `error` })
        })
    }
  }
